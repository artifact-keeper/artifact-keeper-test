...
#              (GREEN "required scope: write"); cancel -> 403 ("delete"). On the
#              vulnerable baseline these return 2xx (RED — the push succeeds).
...
mint_token() { # SCOPE_JSON  -> prints token
  ...
  curl -s $CURL_TIMEOUT -X POST "${BASE_URL}/api/v1/auth/tokens" \
    ...
    -d "{\"name\":\"5f2q-${scopes//[^a-z]/}-${SUF}\",\"scopes\":${scopes}}" 2>/dev/null \
    | jq -r '.token // empty' 2>/dev/null || true
}
READ_TOKEN="$(mint_token '["read"]')"
WRITE_TOKEN="$(mint_token '["write"]')"
if [ -z "$READ_TOKEN" ] || [ -z "$WRITE_TOKEN" ]; then
  begin_test "setup: mint read + write scoped tokens"
  fail "could not mint scoped tokens (read='${READ_TOKEN:0:8}' write='${WRITE_TOKEN:0:8}')"
  end_suite
fi
...
# ---------------------------------------------------------------------------
begin_test "BOUNDARY: read-scoped token POST /uploads (create_session) -> 403 required-scope:write"
...
begin_test "BOUNDARY: read-scoped token PATCH /uploads/:id (upload_chunk) -> 403 required-scope:write"
...
begin_test "BOUNDARY: read-scoped token PUT /uploads/:id/complete -> 403 required-scope:write"
if [ -n "$SESSION_ID" ]; then
