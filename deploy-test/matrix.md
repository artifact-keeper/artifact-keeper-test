...
| 17 | Hosted Debian Release metadata customization — #2489 | filesystem + client.apt | custom apt_origin/label/version/description render into served dists/{dist}/Release; default repo → artifact-keeper + omitted Version/Description; real apt install still works | debian-release | COVERED (PKT-B) |
| 18 | RPM curation trusted GPG key, fail-closed — #2568 | upstreams=rpm-gpg + client.dnf | synchronous curation sync; correct key → ingest>0; wrong key / missing .asc → 0 (fail-closed); dnf repo_gpgcheck=1 fails on wrong key | rpm-curation-gpg | COVERED (PKT-E) |
| 19 | Proxy-cache first-serve download count — #2537 | upstreams=raworigin (rpm remote) | cold proxy serve → proxy_download_statistics count==1 (pre-fix 0), monotonic, HEAD no-increment (DB-surface; reader unwired to a route — product finding) | proxy-downloadcount | COVERED (PKT-F) |
| 20 | Scanner not_applicable row collapse — #2471 | scanners.trivy | .bin artifact → 3 not_applicable rows folded to ONE summary row in the scan-results API (collapsed_not_applicable_count); DB holds >=2 raw | scanner-collapse | COVERED (PKT-F) |
